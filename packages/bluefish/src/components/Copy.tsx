...
export const Copy = forwardRef((props: RefProps, ref: any) => {
  ...
  const [clonedElement, setClonedElement] = useState<SVGElement | null>(null);

  const coordRef = useRef<CoordinateTransform>({});
    ...
    (): Measurable => ({
      ...
      measure(constraints: Constraints): NewBBoxClass {
        ...
        try {
          ...
          const measurableBBox = measurable.current.measure(constraints);
          return new NewBBoxClass(
            {
              // TODO: this is a hack! if the ref isn't already positioned, we just set it to the
              // origin
              // the fix might just be to require these values to be set or else it breaks
              // but maybe the fix is to have components like Row and Col set these values before
              // measuring the next entry.
              left: measurableBBox.left ?? 0,
              top: measurableBBox.top ?? 0,
              right: measurableBBox.right,
              bottom: measurableBBox.bottom,
              width: measurableBBox.width,
              height: measurableBBox.height,
              coord: otherTransformInThisCoordinateSystem,
            },
            {
              coord: (coord) => {
                console.log('[copy] coord set to', coord);
  ...
  return (
    ...
    scale(${coordRef.current?.scale?.x ?? 1} ${coordRef.current?.scale?.y ?? 1})`}
      ref={(node: SVGGElement) => {
        // console.log('[copy] rendering', node, measurable.current);
        if (node && measurable.current && measurable.current.domRef) {
          // if the element doesn't exist, append it
          // if the element does exist, but the domRef has changed, replace it
          // otherwise do nothing
          if (clonedElement === null) {
            // console.log('[copy] cloning', measurable.current.domRef);
            const clone = measurable.current.domRef.cloneNode(true) as SVGElement;
            node.appendChild(clone);
            setClonedElement(clone);
          } else if (!clonedElement.isEqualNode(measurable.current.domRef)) {
            // console.log('[copy] replacing', cloned, measurable.current.domRef);
            const clone = measurable.current.domRef.cloneNode(true) as SVGElement;
            node.replaceChild(clone, clonedElement);
            setClonedElement(clone);
          }
        }
      }}
