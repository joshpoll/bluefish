...
import { Rect, Rect as Rect2 } from '../components/Rect2';
...
export const TreeSymbol = withBluefish(function _Tree({ data }: { data: TreeData }) {
  ...
  const node = useSymbol('node-' + name);
  const subtreesName = useSymbol('subtrees-' + name);
    ...
    <Group2>
      ...
      {(subtrees || []).map((child, i) => (
        <Connector $from={'bottomCenter'} $to={'topCenter'} stroke={'black'} strokeWidth={2}>
          <Ref to={node} />
          <Ref to={lookup(childNames[i], 'node-' + child.name)} />
        </Connector>
      ))}
    </Group2>
  );
