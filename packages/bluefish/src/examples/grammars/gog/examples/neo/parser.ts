...
export function parseTagString(str: string): Array<Node<string>> {
  ...
  function parseCategoryColon(): Array<Node<string>> | undefined {
    if (tokens[i + 1] === ':') {
  ...
  function parseText(): string {
    ...
    throw new Error(`Expected text, got ${tokens[i]}.`);
  }
  ...
  function parseMulti(): Array<Node<string>> | undefined {
    if (tokens[i] === '[') {
      eat('[');
  ...
  function parseListComma(): Array<Node<string>> | undefined {
    if (tokens[i + 1] === ',') {
      const nodes = parsePath();
      eat(',');
      nodes.push(...parseList());
