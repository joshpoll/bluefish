...
export class Matrix {
  ...
  slice(actual: RangeIndex, observed: RangeIndex): NdArray {
    return this.freqs.lo(actual.start, observed.start).hi(actual.end - actual.start, observed.end - observed.start);
  }
  ...
  totalRow(row: Entry): number {
    const slice = this.slice(row.data, {
      start: 0,
      end: this.freqs.shape[1],
    });
    return ops.sum(slice);
  }
