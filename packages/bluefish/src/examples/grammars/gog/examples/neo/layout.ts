...
export function layoutCompact<T>(pred: PredFn<T>, hierarchy: Node<T>): Array<LayoutedNode<T>> {
  ...
  const layout: Array<LayoutedNode<T>> = [];
  const isBottom = (node: Node<T>): boolean => node.isLeaf() || !pred(node);
  hierarchy.fullorder(
    (n) => {
      // pre-order
      layout.push({ node: n, pos: [currPos[0], currPos[1]], span: -1 });
      currPos[0] += 1;
...
export function layoutClassic<T>(pred: PredFn<T>, hierarchy: Node<T>): Array<LayoutedNode<T>> {
  ...
  hierarchy.fullorder(
    ...
    (n) => {
      // post-order
      if (!isBottom(n)) {
        const nc = (n.children || []).length + (n.children || []).reduce((acc, c) => acc + numDescend.get(c), 0);
        numDescend.set(n, nc);
      }
      currPos[0] -= 1;
    },
