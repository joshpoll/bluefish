...
export const line = <T,>(data: T[], { x, y, color }: { x: string; y: string; color: string }): Mark => ({
  data,
  encodings: { x, y, color },
  scale: (
    channels: { X: number[]; Y: number[]; COLOR: string[] },
    scales: { xScale: (x: number) => number; yScale: (y: number) => number; colorScale: (color: string) => string },
    _dimensions: any,
  ) => {
  ...
  render: (data: { stroke: string; points: { x: number; y: number }[] }) => {
    return (
      <Path
        d={d3Line().curve(curveCatmullRom)(data.points.map(({ x, y }) => [x, y])) ?? ''}
        fill={'none'}
        stroke={data.stroke}
        strokeWidth={1.5}
        strokeLinecap={'round'}
...
export const Line: React.FC<{ data?: any[]; encodings: { x: string; y: string; color: string } }> = forwardRef(
  (props, ref) => {
